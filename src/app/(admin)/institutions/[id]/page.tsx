...
export default function InstitutionDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        fetch(`/api/v1/institutions/${id}`)
            ...
            .catch(() => router.replace('/institutions'))
    }, [id, router])

    async function handleUpdate(values: InstitutionFormValues) {
        ...
        <div className="max-w-2xl space-y-6">
            ...
            {deleteConfirm && (
                <div className="rounded-xl border border-red-200 bg-red-50 p-4 flex items-start gap-3">
                    ...
                    <div>
                        <p className="text-sm font-medium text-red-700">Excluir instituição?</p>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                {editMode ? (
                    <InstitutionForm
                        defaultValues={{ name: data.name, city: data.city, active: data.active }}
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Cidade</p>
                                <p className="mt-0.5 text-slate-800">{data.city || '—'}</p>
                            </div>
                        </div>
